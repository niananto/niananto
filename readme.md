<p align="center">
  <img src="https://raw.githubusercontent.com/niananto/niananto/master/assets/cover2.jpg" alt="cover banner" width="100%" />
</p>

<h1 align="center">Hi, I'm Nazmul Islam Ananto <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="35"></h1>

<p align="center">
  <b>Teacher • Researcher • Software Engineer • DevOps & Automation Architect</b>
</p>

<p align="center">
  <a href="https://niananto.com" target="_blank"><img alt="Blogfolio" src="https://img.shields.io/badge/Blogfolio-niananto.com-blue?style=flat-square&logo=google-chrome"></a>
  <a href="https://instagram.com/ananto___" target="_blank"><img alt="Instagram" src="https://img.shields.io/badge/-Instagram-181717?style=flat-square&logo=instagram"></a>
  <a href="https://www.linkedin.com/in/nazmulislamananto" target="_blank"><img alt="LinkedIn" src="https://img.shields.io/badge/-LinkedIn-0077b5?style=flat-square&logo=linkedin"></a>
</p>

---

### 🚀 About Me

- 📚 Adjunct Lecturer @ CSE, BUET
- 🧑‍💻 Software Engineer @ Zelf AI (NY, Remote)
- 🎓 CSE Graduate, BUET - Dean’s List
- 🏆 Zelf Hackathon 1.0 Winner
- 🌏 Building real-world automation, indexing, and social-analysis services at scale
- ⚡ Automating infrastructure, CI/CD pipelines, and real-world data solutions at scale
- 📝 Researcher exploring LLMs, Software Engineering and Security

---

### 🌟 Featured Projects

| Project | Description | Links |
|---|---|---|
| **Online News Scraper** | Real-time scraper/aggregator for 30+ globally recognized news portals with dynamic keyword search | [Code](https://github.com/niananto/online-news-scrapers) |
| **Stock Trader** | Machine Learning-based stock trading agent, trained on real DSE data | [Code](https://github.com/niananto/Stock-Trader) • [Presentation](https://github.com/niananto/Stock-Trader/tree/main/Presentation) |
| **Maps ’n Bags** | Itinerary planner with travel suggestions, diary, and budgeting | [Code](https://github.com/orgs/maps-n-bag/repositories) |
| **Red Light Green Light** | Adaptive traffic light and train detection using Arduino | [Demo Video](https://youtu.be/9mWLkRmKQEA) • [Code](https://github.com/niananto/red-light-green-light) |
| **Withered Away** | Full-stack old home management system (ExpressJS, PostgreSQL) | [Frontend](https://github.com/IshikaIme/withered-away-front) • [Backend](https://github.com/niananto/withered-away-backend-postgres) |
| **Asteroids** | Level-based game developed for GameJam 2023 (Top 5/68) | [Play Now](https://asteroids-gamejam.netlify.app/) |

---

### 🛠️ Tech Stack

![GCP](https://img.shields.io/badge/-GCP-4285F4?logo=google-cloud&logoColor=fff&style=flat)
![Kubernetes](https://img.shields.io/badge/-Kubernetes-326CE5?logo=kubernetes&logoColor=fff&style=flat)
![Docker](https://img.shields.io/badge/-Docker-2496ED?logo=docker&logoColor=fff&style=flat)
![Terraform](https://img.shields.io/badge/-Terraform-7B42BC?logo=terraform&logoColor=fff&style=flat)
![GitHub Actions](https://img.shields.io/badge/-GitHub%20Actions-2088FF?logo=github-actions&logoColor=fff&style=flat)
![Linux](https://img.shields.io/badge/-Linux-333?logo=linux&logoColor=fff&style=flat)
![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=fff&style=flat)
![Javascript](https://img.shields.io/badge/-JavaScript-f7df1e?logo=javascript&logoColor=222&style=flat)
![React](https://img.shields.io/badge/-React-61DAFB?logo=react&logoColor=222&style=flat)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?logo=postgresql&logoColor=fff&style=flat)
![Selenium](https://img.shields.io/badge/-Selenium-43B02A?logo=selenium&logoColor=fff&style=flat)

<details>
  <summary>See full skillset</summary>

- **Cloud/DevOps:** GCP, Docker, Kubernetes, GitHub Actions, Terraform, Linux Shell
- **Languages:** Bash, C, C++, Java, Python, CSS, Javascript, SQL, x86 Assembly, PHP, LATEX
- **Frameworks & Tools:** Flask, FastAPI, Node.js, React, Express, Material UI, Selenium, Playwright, Appium, Git, p5.js, Flex, Bison
- **Databases:** PostgreSQL, MongoDB, MySQL, Oracle, Firebase
- **Security & Debugging:** (strong foundation) Reverse Engineering, ADB, Burp Suite, Mitmproxy, Wireshark, Ghidra
- **ML/Data:** Numpy, Pandas, LangChain, OpenCV, PyTorch, Scikit-learn, SciPy, NLTK, Matplotlib, Seaborn

</details>

---

### ✨ Achievements

- 🥇 **1st/400** Zelf Hackathon 1.0 (2024)
- 🎮 **Top 5/68** GameJam 2023 (IEEE CS, BUET) — "Asteroids"
- 🏅 Dean’s List, BUET (CGPA 3.91)
- 📖 Published in Frontiers: *A Riverbank Erosion Early Warning System: A Case Study of the Braided Brahmaputra‑Jamuna River*

---

### 🌐 Community & Leadership

- 💻 **Director of Marketing & Communication** — BUET Cyber Security Club  
  _Organized national CTFs, promoted cybersecurity in Bangladesh_
- ✍️ **President of Writing & Publication** — Notre Dame English Club  
  _Editor, event organizer, and magazine publisher_

---

### 🎨 Fun Facts

- 🛠️ Always looking for new ways to make infrastructure reliable, fast, and fun
- 🔍 Every “impossible” challenge is just one clever script away

---

<p align="center">
  <b>Let’s connect, collaborate, and build something epic in the cloud!</b>
  <br>
  <a href="https://niananto.com">niananto.com</a> • 
  <a href="https://www.linkedin.com/in/nazmulislamananto">LinkedIn</a> • 
  <a href="https://instagram.com/ananto___">Instagram</a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=niananto&label=Profile%20views&color=0e75b6&style=flat" alt="profile views" />
</p>
